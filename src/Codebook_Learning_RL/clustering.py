...
def clustering_method(
    ...
    if mode == "dft":
        ...
        kmeans = KMeans(n_clusters=num_cluster, random_state=0).fit(
            np.transpose(user_feature_dft)
        )
        ...
        for kk in range(num_random_beam):
            for jj in range(num_ant):
                F[kk, jj * 2 : (jj + 1) * 2] = u_circ_pts[
                    np.random.randint(0, 2**num_bit), :
                ]
        user_vec = bf_gain_cal(F, ch)
        user_feature_vec = corr_mining(user_vec)
        kmeans = KMeans(n_clusters=num_cluster, random_state=0).fit(
            np.transpose(user_feature_vec)
        )
        return F, kmeans
    elif mode == "best":
        s_beam = sensing_beam
