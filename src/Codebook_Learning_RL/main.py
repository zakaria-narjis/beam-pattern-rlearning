...
if __name__ == "__main__":
    ...
    with torch.cuda.device(options["gpu_idx"]):
        ...
        for beam_id in range(options["num_NNs"]):
            actor_net_list.append(
                Actor(options["num_ant"], options["num_ant"])
            )
            actor_net_t_list.append(
                Actor(options["num_ant"], options["num_ant"])
            )
            ...
            actor_net_t_list[beam_id].load_state_dict(
                actor_net_list[beam_id].state_dict()
            )
            critic_net_list[beam_id].apply(init_weights)
            critic_net_t_list[beam_id].load_state_dict(
                critic_net_list[beam_id].state_dict()
            )
